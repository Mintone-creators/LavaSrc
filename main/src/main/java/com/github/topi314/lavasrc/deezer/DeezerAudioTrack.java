...
public class DeezerAudioTrack extends ExtendedAudioTrack {
	...
	private LicenseToken generateLicenceToken(String arl) throws IOException {
		...
		return new LicenseToken(json.get("results").get("USER").get("OPTIONS").get("license_token").text(), json.get("results").get("checkForm").text());
	}
	...
	public enum TrackFormat {
		FLAC(true, FlacAudioTrack::new), MP3_320(true, Mp3AudioTrack::new), MP3_256(true, Mp3AudioTrack::new), MP3_128(false, Mp3AudioTrack::new), MP3_64(false, Mp3AudioTrack::new), AAC_64(true, MpegAudioTrack::new); // not sure if this one is so better to be safe.
